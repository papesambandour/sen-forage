...
class Etudiant  {
    ...
    static gets(cb){
        connexion.query('SELECT * FROM etudiant ORDER BY nom asc',
            [],function (err,result) {
                if(err)
                {
                    throw err ;
    ...
    static getsMore(limit,offset,cb){
        connexion.query('SELECT * FROM etudiant ORDER BY nom asc LIMIT '+limit +' OFFSET  '+offset,
            [],function (err,result) {
                if(err)
                {
                    throw err ;
    ...
    static getsSearch(limit,offset,item,cb)

    {
       // item = toUpperCase(item)
        connexion.query('SELECT * FROM etudiant   WHERE upper(mat) like"'+item+'%" OR upper (nom)  like"%'+item+'%" OR upper(prenom)  like"%'+item+'%" or upper( class)  like"%'+item+'%" ORDER BY prenom asc LIMIT '+limit +' OFFSET '+ offset,
            [],function (err,result) {
                if(err)
                {
                    throw err ;
